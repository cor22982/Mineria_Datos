...
# Primero obtendremos la relacion entre mujeres e ingresos.
...
# Ahora obtendremos la popularidad de Mujeres
ggplot(movies, aes(x = as.numeric(castMenAmount), y = popularity)) +
  ...
  scale_x_continuous(limits = c(0, 100)) +
  theme_minimal()
#Ahora obtendremos la popularidad.de Hombres
...
ggplot(movies, aes(x = as.numeric(castWomenAmount), y = popularity)) +
  ...
  labs(title = "Relación entre la cantidad de actores mujeres y los ingresos",
       x = "Cantidad de Actores Mujeres",
       y = "Popularidad") +
    scale_x_continuous(limits = c(0, 100)) +  # Limitar el rango en el eje X
  theme_minimal()
...
top_20_movies <- movies %>%
  ...
  head(20) %>%
  select(title, voteAvg, director)
print("20 peliculas mas votas y sus directores")
print(top_20_movies)

#. 4.11 ¿Cómo se correlacionan los presupuestos con los ingresos?
ggplot(movies, aes(x = as.numeric(budget), y = revenue)) +
  geom_point(alpha = 0.5, color = "blue") +
  geom_smooth(method = "lm", color = "red") +
  labs(title = "Relación entre el presupuesto e ingresos ",
       x = "Presupuesto",
       y = "Ingresos") +
  theme_minimal()
